...
// ---------------------------------------------------------------------------
// Tamagotchi-style care stats (a dsh-pet extension, not a Codex feature).
// hunger / mood are 0..100 and decay on WALL-CLOCK time, so leaving the app
// closed still counts. Feed restores hunger, play restores mood.
// ---------------------------------------------------------------------------

/** Decay rates: hunger empties in ~8h, mood in ~6h. */
export const STAT_DECAY_PER_HOUR = Object.freeze({ hunger: 12.5, mood: 100 / 6 });

/** Restoration per interaction. */
export const STAT_RESTORE = Object.freeze({ feed: 30, play: 25 });

/** Below this the pet hints it wants attention (idle-state badge). */
export const STAT_LOW_THRESHOLD = 20;

const clampStat = (v) => Math.min(100, Math.max(0, Math.round(v * 10) / 10));

/** Normalize/derive current stats from a stored record, applying decay. */
export function decayStats(stored, now = Date.now()) {
...
/** Feed: +STAT_RESTORE.feed hunger (capped at 100), decay applied first. */
export function feedStats(stored, now = Date.now()) {
  const cur = decayStats(stored, now);
  return { ...cur, hunger: clampStat(cur.hunger + STAT_RESTORE.feed) };
}

/** Play: +STAT_RESTORE.play mood (capped at 100), decay applied first. */
export function playStats(stored, now = Date.now()) {
  const cur = decayStats(stored, now);
  return { ...cur, mood: clampStat(cur.mood + STAT_RESTORE.play) };
}

/** Idle-state hint: 'hungry' | 'bored' | null (hunger outranks boredom). */
export function statHint(stored, now = Date.now()) {
  const cur = decayStats(stored, now);
