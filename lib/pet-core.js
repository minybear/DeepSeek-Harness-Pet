...
/**
 * Priority-ordered pet state from a session snapshot.
 * `celebrateUntil` (epoch ms) makes a just-finished run celebrate before
 * settling back to idle.
 */
export function derivePetState({ running, waiting, error, celebrateUntil }) {
...
/** The states this plugin can drive (V1 atlas rows only). */
export const DRIVABLE_STATES = Object.freeze(['idle', 'running', 'waiting', 'failed', 'jumping', 'waving', 'review']);
