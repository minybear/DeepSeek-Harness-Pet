...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PetOverlay(props) {
			...
			// session switch: wave hello (Codex SessionStart -> waving)
			const prevSession = React.useRef(null);
			React.useEffect(() => {
				if (currentId && currentId !== prevSession.current) setWaveUntil(Date.now() + 1700);
				if (currentId) prevSession.current = currentId;
			}, [currentId]);
