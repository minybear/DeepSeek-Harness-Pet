...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// --- care stats (mirror of pet-core.js; parity-tested) ------------------
		const STAT_DECAY_PER_HOUR = Object.freeze({ hunger: 12.5, mood: 100 / 6 });
		const STAT_RESTORE = Object.freeze({ feed: 30, play: 25 });
		const STAT_LOW_THRESHOLD = 20;
		const clampStat = (v) => Math.min(100, Math.max(0, Math.round(v * 10) / 10));
		function decayStats(stored, now = Date.now()) {
			const s = stored && typeof stored === "object" ? stored : {};
			const updatedAt = typeof s.updatedAt === "number" ? s.updatedAt : now;
			const hours = Math.max(0, (now - updatedAt) / 3_600_000);
			return {
				hunger: clampStat((typeof s.hunger === "number" ? s.hunger : 100) - hours * STAT_DECAY_PER_HOUR.hunger),
				mood: clampStat((typeof s.mood === "number" ? s.mood : 100) - hours * STAT_DECAY_PER_HOUR.mood),
				updatedAt: now,
			};
		}
		function feedStats(stored, now = Date.now()) {
			const cur = decayStats(stored, now);
			return { ...cur, hunger: clampStat(cur.hunger + STAT_RESTORE.feed) };
		}
		function playStats(stored, now = Date.now()) {
			const cur = decayStats(stored, now);
			return { ...cur, mood: clampStat(cur.mood + STAT_RESTORE.play) };
		}
		function statHint(stored, now = Date.now()) {
			const cur = decayStats(stored, now);
			if (cur.hunger < STAT_LOW_THRESHOLD) return "hungry";
			if (cur.mood < STAT_LOW_THRESHOLD) return "bored";
			return null;
		}
		...
		const BUILTIN_PETS = Object.freeze({
			...
			graywhale: {
				displayName: "灰鲸 (Harness)",
				description: "The DeepSeek Harness gray whale, mirroring the agent's running state.",
				shape: "whale",
				palette: { body: "#8b95a5", belly: "#dde3ec", accent: "#5b6472" },
			},
			bluewhale: {
				displayName: "蓝鲸 (DeepSeek)",
				description: "The DeepSeek brand blue whale, mirroring the agent's running state.",
				shape: "whale",
				palette: { body: "#4d6bfe", belly: "#c9d6ff", accent: "#2f49c8" },
			},
		});
		...
		function drawEye(ctx, x, y, mode, px, es = 1) {
			ctx.save();
			ctx.translate(x, y);
			ctx.scale(es, es);
			ctx.fillStyle = C_EYE;
		...
		// ---------------------------------------------------------------------
		// Whale shape (DeepSeek / DeepSeek Harness logos). Side-view creature:
		// body ellipse + head circle + tail fluke, blowhole spout on idle,
		// bubbles while working, flipper wave, breach on jumping.
		// Drawn facing right; callers mirror with ctx.scale(-1, 1) via `facing`.
		// ---------------------------------------------------------------------
		const C_WATER = "#9fd8ff";

		function drawWhale(ctx, p, pal) {
			// p: {cx, cy, s(scaleY), y(offset), eyes, px(pupil x, local/facing coords),
			//     facing(1 right / -1 left), spout(0..1), droopSpout(bool), flipper(bool),
			//     tailWag(-1..1), flat(bool), bubbles(bool), frame(f), total}
			const cx = p.cx, cy = p.cy + p.y;
			ctx.save();
			ctx.translate(cx, cy);
			ctx.scale(p.facing || 1, 1);

			// tail fluke (drawn first, behind the body)
			const wag = (p.tailWag || 0) * 0.22;
			ctx.fillStyle = pal.body;
			ctx.save();
			ctx.translate(-23, -1);
			ctx.rotate(-0.55 + wag);
			ctx.beginPath(); ctx.ellipse(-6, -4, 10, 4.4, 0, 0, Math.PI * 2); ctx.fill();
			ctx.restore();
			ctx.save();
			ctx.translate(-23, 1);
			ctx.rotate(0.55 + wag);
			...
			ctx.scale(1, p.s);
			ctx.fillStyle = pal.body;
			ctx.beginPath(); ctx.ellipse(-2, 0, 24, 16, 0, 0, Math.PI * 2); ctx.fill();
			ctx.beginPath(); ctx.arc(13, -3, 14, 0, Math.PI * 2); ctx.fill();
			// outline
			ctx.strokeStyle = C_OUTLINE; ctx.lineWidth = 2.2;
			ctx.beginPath(); ctx.ellipse(-2, 0, 24, 16, 0, Math.PI * 0.62, Math.PI * 1.9); ctx.stroke();
			ctx.beginPath(); ctx.arc(13, -3, 14, Math.PI * 1.1, Math.PI * 1.95); ctx.stroke();
			// belly
			ctx.fillStyle = pal.belly;
			ctx.beginPath(); ctx.ellipse(3, 7, 15, 6.5, 0, 0, Math.PI * 2); ctx.fill();
			ctx.restore();

			// pectoral fin (flipper); raised when waving
			ctx.fillStyle = pal.accent;
			if (p.flipper) {
				ctx.save(); ctx.translate(4, 0); ctx.rotate(-1.15);
				ctx.beginPath(); ctx.ellipse(0, 0, 8, 3.4, 0, 0, Math.PI * 2); ctx.fill();
				ctx.restore();
			} else {
				ctx.save(); ctx.translate(0, 10); ctx.rotate(0.45);
				ctx.beginPath(); ctx.ellipse(0, 0, 7, 3, 0, 0, Math.PI * 2); ctx.fill();
				ctx.restore();
			}

			// eye (whale eyes are small)
			drawEye(ctx, 13, -8, p.eyes, p.px, 0.7);

			// mouth: gentle smile
			ctx.strokeStyle = C_OUTLINE; ctx.lineWidth = 1.6; ctx.lineCap = "round";
			ctx.beginPath(); ctx.arc(15, -1, 5, 0.25, Math.PI * 0.7); ctx.stroke();

			// blowhole spout (idle breathing); droops to the side when failed
			if (p.droopSpout) {
				ctx.strokeStyle = C_WATER; ctx.lineWidth = 2; ctx.lineCap = "round";
				ctx.beginPath(); ctx.moveTo(6, -15); ctx.quadraticCurveTo(12, -13, 13, -7); ctx.stroke();
			} else if (p.spout > 0) {
				const t = p.spout;
				ctx.strokeStyle = C_WATER; ctx.lineWidth = 2; ctx.lineCap = "round";
				ctx.beginPath(); ctx.moveTo(6, -15); ctx.lineTo(6, -15 - 9 * t); ctx.stroke();
				ctx.fillStyle = C_WATER;
				ctx.beginPath(); ctx.arc(6, -17 - 10 * t, 1.4 + 1.6 * t, 0, Math.PI * 2); ctx.fill();
				ctx.beginPath(); ctx.arc(3, -15 - 12 * t, 1 + 1.2 * t, 0, Math.PI * 2); ctx.fill();
				ctx.beginPath(); ctx.arc(9, -15 - 12 * t, 1 + 1.2 * t, 0, Math.PI * 2); ctx.fill();
			}

			// bubbles while working
			if (p.bubbles) {
				const rise = (p.frame / Math.max(1, p.total)) * 6;
				ctx.strokeStyle = C_WATER; ctx.lineWidth = 1.4;
				ctx.beginPath(); ctx.arc(19, -12 - rise, 1.6, 0, Math.PI * 2); ctx.stroke();
				ctx.beginPath(); ctx.arc(22, -18 - rise, 2.1, 0, Math.PI * 2); ctx.stroke();
				ctx.beginPath(); ctx.arc(18, -24 - rise, 1.3, 0, Math.PI * 2); ctx.stroke();
			}
			ctx.restore();
		}

		function drawCell(ctx, col, row, state, f, total, pal, shape) {
			...
			// whale params: facing is 1 (right) by default; px is in local facing
			// coords so pupils always look "forward" after the mirror.
			...
			switch (state) {
				case "idle": {
					blob.s = 1 + 0.03 * Math.sin((f / total) * Math.PI * 2);
					if (f % 3 === 2) blob.eyes = "blink";
					whale.s = 1 + 0.03 * Math.sin((f / total) * Math.PI * 2);
					if (f % 3 === 2) whale.eyes = "blink";
				...
				case "running-left": {
					...
					whale.facing = -1;
					whale.tailWag = f % 2 === 0 ? 1 : -1;
					whale.px = 1.5; // forward in local coords -> left on screen after flip
					break;
				}
				case "waving": {
					blob.arm = true;
					blob.s = f % 2 === 0 ? 1.03 : 1;
					whale.flipper = true;
					whale.s = f % 2 === 0 ? 1.03 : 1;
					break;
				}
				case "jumping": {
					const seq = [0, -14, -24, -14, 0];
					blob.y = seq[f] ?? 0;
					blob.s = f === 0 ? 1.08 : f === 4 ? 0.9 : 1;
					// a whale breach: bigger arc, slight tilt at the peak
					const wseq = [0, -16, -28, -16, 0];
					whale.y = wseq[f] ?? 0;
					whale.s = f === 0 ? 1.06 : f === 4 ? 0.92 : 1;
					break;
				}
				case "failed": {
					blob.flat = true;
				...
				case "running": {
					blob.eyes = "narrow";
					blob.gear = true;
					blob.s = 1 + 0.02 * Math.sin((f / total) * Math.PI * 2);
					blob.px = 0.5;
					whale.eyes = "narrow";
					whale.bubbles = true;
					whale.s = 1 + 0.02 * Math.sin((f / total) * Math.PI * 2);
					whale.px = 0.5;
					break;
				}
				case "review": {
			...
			if (shape === "whale") drawWhale(ctx, whale, pal);
			else drawCreature(ctx, blob, pal);
		...
		function buildDefaultAsset(paletteKey = "dee") {
			...
			const shape = def.shape === "whale" ? "whale" : "blob";
			...
			for (let row = 0; row < order.length; row++) {
				...
				for (let f = 0; f < count; f++) drawCell(ctx, f, row, state, f, count, pal, shape);
			}
		...
		function PetOverlay(props) {
			...
			// care stats: stored record {hunger, mood, updatedAt}; displayed
			// values are always decay-derived at render time.
			const [stats, setStats] = React.useState(() => decayStats(readJSON(LS.stats, null)));
			...
			React.useEffect(() => { writeJSON(LS.stats, stats); }, [stats]);
			...
			// care-stat hint badge: only when the pet would otherwise be idle
			// (session states and interactions always outrank the hint).
			const curStats = decayStats(stats, now);
			const hint = (label == null && state === "idle") ? statHint(stats, now) : null;
			const hintLabel = hint === "hungry" ? "🍖 饿了" : hint === "bored" ? "🎾 想玩" : null;
			const finalLabel = label != null ? label : hintLabel;
			...
			const doEat = () => { setEatUntil(Date.now() + 2200); setStats((cur) => feedStats(cur)); setMenuOpen(false); };
			const doPlay = () => { setPlayUntil(Date.now() + 2600); setStats((cur) => playStats(cur)); setMenuOpen(false); };
			...
			return React.createElement("div", {
				...
				menuOpen && React.createElement("div", {
					...
					React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4, minWidth: 150 } },
						[["🍖 饱食", curStats.hunger, "#e35d5d"], ["😊 心情", curStats.mood, "#5b8def"]].map(([name, value, color]) =>
							React.createElement("div", { key: name, style: { display: "flex", alignItems: "center", gap: 6 } },
								React.createElement("span", { style: { font: "11px system-ui, sans-serif", color: "var(--dsw-alias-label-primary, #333)", width: 52 } }, name),
								React.createElement("div", { style: { flex: 1, height: 6, borderRadius: 3, background: "var(--dsw-alias-border-l1, #e5e6eb)", overflow: "hidden" } },
									React.createElement("div", { style: { width: `${Math.round(value)}%`, height: "100%", background: color, borderRadius: 3 } })
								),
								React.createElement("span", { style: { font: "10px system-ui, sans-serif", color: "var(--dsw-alias-label-secondary, #888)", width: 26, textAlign: "right" } }, String(Math.round(value)))
							)
						)
					),
				...
				finalLabel != null && React.createElement("div", {
					style: {
		...
		exports.__petCore = {
			...
			STAT_DECAY_PER_HOUR, STAT_RESTORE, STAT_LOW_THRESHOLD, decayStats, feedStats, playStats, statHint,
		};
